...
var optab = []Optab{
	...
	{AMOVV, C_GOTADDR, C_NONE, C_REG, C_NONE, 63, 8, 0, sys.Loong64, 0},
	...
	{AMOVV, C_FCCREG, C_NONE, C_REG, C_NONE, 64, 4, 0, sys.Loong64, 0},
	{AMOVV, C_REG, C_NONE, C_FCCREG, C_NONE, 65, 4, 0, sys.Loong64, 0},
...
func (c *ctxt0) asmout(p *obj.Prog, o *Optab, out []uint32) {
	...
	case 56: // mov r, tlsvar: IE model ==> (pcaddu12i + ld.d)tlsvar@got + add.d + st.d
		o1 = OP_IR(c.opir(APCADDU12I), uint32(0), uint32(REGTMP))
		rel := obj.Addrel(c.cursym)
		rel.Off = int32(c.pc)
		rel.Siz = 4
		...
		rel2.Siz = 4
		rel2.Sym = p.To.Sym
		rel2.Type = objabi.R_LOONG64_TLS_IE_PCREL_LO
		rel2.Add = 0x0
		o3 = OP_RRR(c.oprrr(AADDVU), uint32(REG_R2), uint32(REGTMP), uint32(REGTMP))
		o4 = OP_12IRR(c.opirr(p.As), uint32(0), uint32(REGTMP), uint32(p.From.Reg))

	case 57: // mov tlsvar, r: IE model ==> (pcaddu12i + ld.d)tlsvar@got + add.d + ld.d
		o1 = OP_IR(c.opir(APCADDU12I), uint32(0), uint32(REGTMP))
		rel := obj.Addrel(c.cursym)
		rel.Off = int32(c.pc)
		rel.Siz = 4
	...
	case 63: // mov sym@GOT, r ==> pcaddu12i + ld.d
	        ...
	        rel2.Add = 0x0
	case 64: // movv c_fcc0, c_reg ==> movcf2gr rd, cj
		a := OP_TEN(8, 1335)
		o1 = OP_RR(a, uint32(p.From.Reg), uint32(p.To.Reg))
	case 65: // movv c_reg, c_fcc0 ==> movgr2cf cd, rj
		a := OP_TEN(8, 1334)
		o1 = OP_RR(a, uint32(p.From.Reg), uint32(p.To.Reg))
	}
